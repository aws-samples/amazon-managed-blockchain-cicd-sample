...
//FxChaincode is the struct that all chaincode methods are associated with
type FxChaincode struct {
}

//Init method is invokved on installation and upgrade
func (s *FxChaincode) Init(stub shim.ChaincodeStubInterface) sc.Response {
	return shim.Success(nil)
}

//Invoke is called when external applications invoke the smart contract
func (s *FxChaincode) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
...
func (s *FxChaincode) createUpdateForexPair(stub shim.ChaincodeStubInterface, args []string) sc.Response {
...
func (s *FxChaincode) getForexPair(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	if len(args) != 2 {
...
func main() {
	...
	if err != nil {
		panic("Error creating new FxChaincode: " + err.Error())
	}
}
